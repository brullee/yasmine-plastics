...
export default async function ProductsPage({ params, searchParams }: Props) {
        ...
        <div className="bg-gray-50 dark:bg-gray-950 min-h-[60vh]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <div className="mt-10 rounded-xl border-2 border-dashed border-brand-navy/40 dark:border-slate-600 bg-blue-50 dark:bg-slate-800 px-8 py-10 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div>
                <p className="font-semibold text-brand-navy dark:text-white text-base mb-1">
                  {t('cantFind.title')}
                </p>
              ...
              <Link
                href="/contact"
                className="shrink-0 px-6 py-2.5 bg-brand-navy text-white text-sm font-semibold rounded-lg hover:bg-brand-navyDark transition-colors"
              >
                {t('cantFind.cta')}
              </Link>
            </div>
  ...
  // ── Category selected → product grid ─────────────────────────────────────
  ...
  return (
    <>
      ...
      {/* Info bar: breadcrumb left, results right */}
      ...
      <div className="bg-gray-50 dark:bg-gray-950 min-h-[60vh]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <div className="rounded-xl border-2 border-dashed border-brand-navy/40 dark:border-slate-600 bg-blue-50 dark:bg-slate-800 px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <Link
              href="/contact"
              className="shrink-0 px-5 py-2 bg-brand-navy text-white text-xs font-semibold rounded-lg hover:bg-brand-navyDark transition-colors"
            >
              {t('cantFind.cta')}
            </Link>
          </div>
